...
class MoonService
  ...
  def current_season
    cache("current_season:#{date_key}") { season_for(@date.to_date) }
  end

  private

  def cache(key, &block) = Rails.cache.fetch(key, expires_in: 1.day, &block)
  def date_key = @date.to_date.to_s
  ...
  def phase_anchor_time(phase_index:, around:, forward: false)
    ...
    delta_days += SYNODIC_MONTH / 4.0 if forward && delta_days <= 0
    (around + delta_days.days).utc
  end

  def season_for(date)
    y = date.year
    summer_start = Date.new(y - 1, 12, 21)
